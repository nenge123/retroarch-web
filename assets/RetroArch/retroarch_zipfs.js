(function () {
    ...
    this.action['reset-reload'] = async () => {
        //await Promise.all(["cores-vba_next_libretro","cores-mgba_libretro"].map(async key=>await T.removeItem('data-libjs',key)));
        location.reload();
    };
    T.docload(async () => {
        let data = await T.getContent('data-libjs', 'myzipfs');
        if (data) await T.addJS(data);
        Object.defineProperty(T.unitl, 'unZip', {
            value: async function (u8, ARG = {}) {
                let {
                    process,
                    password,
                    packtext
        ...
        if (!window.zip) T.$('.test-result').innerHTML = 'you must upload a zip-fs.js!';
            ...
            T.$('.g-start-list').hidden = false;
            T.$('.test-result').innerHTML = 'load ready! you can test!';
        }
