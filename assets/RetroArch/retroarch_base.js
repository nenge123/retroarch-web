(function () {
    ...
    this.action = {
        ...
        'game-welcome-elm': () => {
            let elm = T.$('.game-ui .g-welcome-result');
            if (elm) {
                let div = T.$ce('div');
                elm.appendChild(div);
                return div;
        ...
        'game-welcome-WriteFile': async (file, elm) => {
            ...
            if (info.unFile === false) {
                ...
                let zipdata = await T.unitl.unFile(contents, text =>{if(elm)elm.innerHTML = text});
