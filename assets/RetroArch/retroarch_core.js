const Nenge = new class{
    ...
    async getItem(store,name,version) {
        ...
        if(result&&result.contents&&result.filesize&&result.filesize>maxsize){
            ...
            await Promise.all(save.map(async entry=>{
                ...
                if(!subresult){throw T.Err(entry+' data is lost');}
                let subidx = parseInt(entry.split(part)[1])||0;
                returnBuf.set(subresult.contents,subidx*maxsize);
            }));
            if(result.type.split('/')[0]=='file'){
                result.contents = new File([returnBuf],keys[0],{type:result.type.split('/').slice(1).join('/')});
    ...
    async setItem(store, name, data,cb) {
        let T=this,db = await T.GET_DB(store),maxsize = T.maxsize,part = T.part,type=data.type||'';
        if((typeof data == 'string'&&data>maxsize/2)||(typeof data.contents == 'string'&&data.contents>maxsize/2)){
            ...
            else data = {'contents':new Uint8Array(await data.arrayBuffer())};
            Object.assign(data,{'type':filetype?'file/'+filetype:type,filesize});
        }
        if(data.hasOwnProperty('version')&&!data.version)delete data.version;
        if(data.contents&&data.contents.length>maxsize){
            let save = {},BufLen = data.contents.byteLength;
            ...
            let result = await Promise.all(
                Object.entries(save).map(async entry=>{
                    ...
                    return await T.setItem(store,key,{
                        'contents':new Uint8Array(data.contents.subarray(start,end)),
                        'filesize':data.filesize,
                        'timestamp':data.timestamp||this.DATE,
                        'type':data.type
                    });
                })
            );
    ...
    async FectchItem(ARG){
        let T=this,
            ...
            result,
            version = ARG.version,headers={},response,
            callback = result=>{
        ...
        if(ARG.store){
            if(contents instanceof Uint8Array||ARG.key!='libdata'){
                await T.setItem(ARG.store,key,{contents,timestamp:new Date,filesize,version,type});
    ...
    unitl = new class {
        ...
        async fetch(ARG){
            if(!ARG.url) throw {url:"url is null"};
            let url = ARG.url,fd,form,data={};
            if(ARG.get){
            ...
            return await fetch(url,data).catch(e=>{throw e});
        }
        async unRAR(u8,cb,password,key,file){
            ...
            return new Promise(complete=>{
                ...
                worker.postMessage(!file?{data:u8,password}:u8);
            });
       ...
       async unFile(u8,cb,password,key){
        ...
        async getDB(DB_NAME,ARG){
            ...
            return new Promise((resolve,reject)=>{
                ...
                req.addEventListener('upgradeneeded',async e =>{
                    ...
                    if(ARG.upgrad){
                        ARG.upgrad({db,'obj':req,'list':Names});
                    }else if(ARG.store&&!Names.contains(ARG.store)){
                        let Store = db.createObjectStore(ARG.store);
                        if(ARG.Index)Store.createIndex(ARG.Index,ARG.Index, {"unique": false});
                    }
                });
        ...
        getItem(transaction,name){
            return new Promise(back=>{transaction.get(name).onsuccess=e=>back(e.target.result);});
        }
        constructor(t){
            Object.defineProperties(this,{
                'T':{get:()=>t},
                'getLibjs':{value:e=>t.getLibjs(e)}
            });
        }
    }(this);
    async getLibjs(file){
        ...
        if(contents){
            if(file=='rar.js'){
                ...
                this.Libjs[file] = new File([contents],file,{'type':this.unitl.gettype(file),'x-content-type-options':'nosniff'});
            }else{
    ...
    transaction(store,db,mode){
        ...
        transaction.onerror = e => {e.preventDefault(); throw transaction.error; }; return transaction.objectStore(store);
    }
    async clearDB(storeName){
        let T=this;if(!storeName)return T.indexedDB.deleteDatabase(T.DB_NAME);
    ...
    ajax(ARG){
        ...
        request.responseType = ARG.type||"arraybuffer";
                    ...
                    switch(request.readyState){
                        ...
                        case 4:
                            if(request.status==500){
                                request.dispatchEvent(new ErrorEvent('error',{message:request.response}));
            ...
            let urldata = ARG.url.split('?'),url=urldata[0],Params,formData;
            if(urldata[1]){
    ...
    docload(f){
        return new Promise(complete=>{
            if(document.readyState =='loading'){
                let x=document,d='DOMContentLoaded',func = ()=>{
                    this.un(x,d,func);
                    f&&f.call(this);
                    complete(true);
                };
